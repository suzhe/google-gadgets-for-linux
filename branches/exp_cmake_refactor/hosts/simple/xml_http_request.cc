...
using ggadget::CaseInsensitiveStringMap;
using ggadget::Connection;
using ggadget::DOMDocument;
using ggadget::Slot0;
using ggadget::TrimString;
using ggadget::XMLHttpRequestInterface;

XMLHttpRequest::XMLHttpRequest(GtkCairoHost *host)
    : host_(host),
      async_(false),
      curl_(NULL),
      curlm_(NULL),
...
bool XMLHttpRequest::Send(const char *data) {
  ...
  if (async_) {
    ...
    // As described in the specification, here don't change the state, but send
...
void XMLHttpRequest::OnIOReady(int fd) {
...
int XMLHttpRequest::SocketCallback(CURL *handle, curl_socket_t socket, int type,
                                   void *user_p, void *sock_p) {
  DLOG("SocketCallback: the socket: %d, type: %d\n", socket, type);
  XMLHttpRequest* this_p = reinterpret_cast<XMLHttpRequest*>(user_p);
...
size_t XMLHttpRequest::ReadCallback(void *ptr, size_t size,
                                    size_t mem_block, void *data) {
  DLOG("XMLHttpRequest: ReadCallback: %d*%d", size, mem_block);
  XMLHttpRequest* this_p = reinterpret_cast<XMLHttpRequest*>(data);
  ASSERT(this_p);
  ...
  strncpy(reinterpret_cast<char *>(ptr), this_p->send_data_.c_str(), real_size);
  this_p->send_data_.erase(0, real_size);
  return real_size;
}

size_t XMLHttpRequest::WriteHeaderCallback(void *ptr, size_t size,
                                           size_t mem_block, void *data) {
...
size_t XMLHttpRequest::WriteBodyCallback(void *ptr, size_t size,
                                         size_t mem_block, void *data) {
...
const char *XMLHttpRequest::GetResponseHeader(const char *header) {
  if (state_ == LOADING || state_ == DONE) {
    CaseInsensitiveStringMap::iterator it = response_headers_map_.find(header);
