...
function AddPluginBox(plugin, index, row, column) {
  ...
  UpdateAddButtonVisualStatus(plugin.button, plugin.download_status);
}
...
function SetDownloadStatus(plugin, status) {
  ...
  UpdateAddButtonVisualStatus(button, status);
}
...
function UpdateAddButtonVisualStatus(button, status) {
  ...
  button.enabled = status != kDownloadStatusAdding;
...
function pluginbox_onmouseout(index) {
  ...
  if (!view.mouseOverElement || view.mouseOverElement.parentElement != box)
    MouseOutPlugin(box, index);
}

function add_button_onmouseover(index) {
  // Won't change status when adding, because the button is disabled in that
  // status and won't fire onmouseover.
