...
static inline bool SVGColorNameCompare(const SVGColorInfo &c1,
                                       const SVGColorInfo &c2) {
  ...
  if (name[0] == '#' && (len == 7 || (len == 9 && alpha))) {
    ...
    if ((len == 7 && sscanf(ptr, "%02x%02x%02x", &r, &g, &b) == 3) ||
      ...
      *color = Color(r / 255.0, g / 255.0, b / 255.0);
      ...
      LOG("Color::FromString(%s): Invalid color value.", name);
      return false;
    }
  }

  SVGColorInfo key = { name, 0, 0, 0, };
  const SVGColorInfo *pos = std::lower_bound(
      kSVGColors, kSVGColors + arraysize(kSVGColors), key, SVGColorNameCompare);

  ASSERT(pos);
  if (strcmp(name, pos->name) == 0) {
    *color = FromChars(pos->r, pos->g, pos->b);
    if (alpha)
      *alpha = 1.0;
    return true;
  }
